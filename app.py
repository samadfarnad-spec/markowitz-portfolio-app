...
# نمایش انتخاب کاربر بالای صفحه
st.markdown(f"### 🔹 مدل انتخابی: **{model_choice}**")
...
# -------------------------------
if st.button("📈 محاسبه پرتفوی بهینه"):
...
# -------------------------------
# پرتفوی کاربر (Custom Portfolio)
# -------------------------------
st.header("🔹 پرتفوی کاربر (وزن‌های دستی)")

user_stock = st.number_input("وزن سهام (کاربر)", 0.0, 1.0, 0.33)
user_bond = st.number_input("وزن درآمد ثابت (کاربر)", 0.0, 1.0, 0.33)
user_gold = st.number_input("وزن طلا (کاربر)", 0.0, 1.0, 0.34)

if st.button("📊 محاسبه پرتفوی کاربر"):
    weights = np.array([user_stock, user_bond, user_gold])
    if not np.isclose(weights.sum(), 1.0):
        st.error("❌ مجموع وزن‌ها باید برابر ۱ باشد.")
    else:
        port_return = expected_returns @ weights
        port_risk = np.sqrt(weights.T @ cov_matrix @ weights)
        sharpe = (port_return) / port_risk if port_risk > 0 else 0

        st.subheader("📊 نتایج پرتفوی کاربر")
        ...
        st.write(f"🔹 بازده پرتفوی: {port_return:.3f}")
        st.write(f"🔹 ریسک پرتفوی: {port_risk:.3f}")
        st.write(f"🔹 نسبت شارپ: {sharpe:.3f}")
